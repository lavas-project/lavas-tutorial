...
> info
>
> 在 `module.rules` 规则中，使用了 `enforce: 'pre'` ，这是为了保证 svg-loader 的执行时机在所有 loader 之前。例如待修改的 `src/app.js` ，也满足下面 babel-loader 的规则，将在 svg-loader 处理之后（注入了使用 ES6 语法的代码）执行。
...
- 向 `src/assets/svg` 文件夹中放入新文件，引入自定义 svg
- 修改 `config/icon.js` 文件的 `icons` ，引入／删除 fontawesome 图标
