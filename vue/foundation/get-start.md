...
PWA 工程构建出的 App 是一个 Web App，我们需要做的是使这样的一个 Web App 在体验上和 Native App 保持一致，并且不会因为安装导致占用用户设备的存储空间（当然，Service Worker 在缓存的过程中会占用部分存储空间，但是可以通过有效策略控制占用空间的大小）。通常我们需要确保我们的工程完成了如下工作。
...
为了实现流畅的 Web App 页面切换以及缩短页面打开和跳转的等待时长，业界在过去的很长时间都在进行着不断的探索。从 Ajax 的问世开始，Web 应用的开发者可以通过一系列的解决方案使得 Web 页面的交互形式越来越复杂和丰富，并且随着 HTML5 / CSS3 的一些新技术和 Web 标准的出现，Web 页面在交互体验上得到进一步的提升。另一边随着智能机的普及以及 Native App 的数量激增和体验不断提升获得越来越多用户的青睐，促使着 Web 开发者脑洞大开的不断的使用新鲜的强大技术朝着 Native App 的优秀设计和体验目标去努力，所以近几年越来越多的类 Native App 的单页 Web 应用 (SPA) 问世，他们在交互形式和用户体验方面和 Native App 几乎保持一致，并且在实现和体验上积累了很多非常高效的解决方案。考虑到异步请求的可缓存性以及用户体验的流畅性，我们在 PWA 工程的实践中首选的工程架构模式是 SPA 模式
...
本教程将一步一步的领着你使用 Lavas 进入 PWA 工程化之旅，我们通过 Lavas 提供的一系列解决方案帮助你完成一个完善的 PWA 应用。
